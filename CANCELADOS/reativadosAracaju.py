...
from dados import QUERY_REATIVADOS_ARACAJU
...
def enviar_email_report(dfs_dict, metricas_dict, destinatarios, com_copia=None, com_copia_oculta=None):
    ...
    msg['Subject'] = f"📊 Relatório de Reativações Aracaju VitalMed - {pd.Timestamp.now().strftime('%d/%m/%Y')}"
...
if __name__ == "__main__":
    ...
    df_reativados = pd.read_sql_query(QUERY_REATIVADOS_ARACAJU, conn)
    conn.close()
    ...
    enviar_email_report(
        ...
        destinatarios="admvendas@vitalmed.com.br",  # Destinatário principal
        com_copia = [
    ...
    #"pssilva@vitalmed.com.br"
        ...
        com_copia_oculta=["laporciuncula@vitalmed.com.br"]  # Bcc (não aparece para outros)
    )
