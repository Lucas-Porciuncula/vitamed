...
from dados import QUERY_REATIVADOS_FEIRA
...
def enviar_email_report(dfs_dict, metricas_dict, destinatarios, com_copia=None, com_copia_oculta=None):
    ...
    msg['Subject'] = f"📊 Relatório de Reativações FSA VitalMed - {pd.Timestamp.now().strftime('%d/%m/%Y')}"
...
if __name__ == "__main__":
    ...
    df_reativados = pd.read_sql_query(QUERY_REATIVADOS_FEIRA, conn)
    conn.close()
