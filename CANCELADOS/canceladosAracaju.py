...
from dados import QUERY_CANCELADOS_ARACAJU
...
def enviar_email_report(dfs_dict, metricas_dict, destinatarios, com_copia=None, com_copia_oculta=None,  cor_header='#1a3c5e'):
    ...
    msg['Subject'] = f"📊 Relatório de Cancelamentos Aracaju Vitalmed - {pd.Timestamp.now().strftime('%d/%m/%Y')}"
...
if __name__ == "__main__":
    ...
    df_cancelados = pd.read_sql(QUERY_CANCELADOS_ARACAJU, conn)
    conn.close()
